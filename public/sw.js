...
const SHELL = ['/games.html', '/assets/css/app.css', '/assets/js/i18n.js', '/assets/js/theme.js', '/assets/js/common.js', '/assets/js/footer.js', '/assets/js/games.js'];
  ...
  if (request.mode === 'navigate' || SHELL.includes(url.pathname)) {
    event.respondWith(
      fetch(request)
        ...
        .catch(async () => {
          // الصفحة تطلب الأصول ببصمة إصدار (‎?v=1.48.0‎) بينما نخزّنها بلا
