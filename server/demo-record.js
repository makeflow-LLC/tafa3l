...
/** سؤالٌ في نشاطٍ تجريبي: نصّه، وصوابه، والخطأ الشائع فيه */
const q = (text, right, mine) => ({ text, right, mine });
 ...
 */
const HARD_FRACTION = q('١/٣ + ١/٦ = ؟', '١/٢', '٢/٩');
const HARD_GCD = q('أكبر قاسم مشترك للعددين ١٨ و٢٤؟', '٦', '٣');
...
const ACTIVITIES = [
  {
    ...
    totalMark: 10,
    questions: [
      q('الكسر ٥/٥ يساوي؟', 'واحد صحيح', 'خمسة'),
      q('٣/٥ − ١/٥ = ؟', '٢/٥', '٢/٠'),
      q('الكسر المكافئ للكسر ٢/٤ هو؟', '١/٢', '٤/٢'),
      q('١/٢ + ١/٤ = ؟', '٣/٤', '٢/٦'),
      q('أيّهما أكبر: ٢/٣ أم ٣/٤؟', '٣/٤', '٢/٣'),
      HARD_FRACTION,
    ],
  },
  {
    ...
    totalMark: 0,
    questions: [
      q('«كتبَ الطالبُ الدرسَ» — نوع الجملة؟', 'فعلية', 'اسمية'),
      q('الفاعل في «قرأ محمدٌ القصةَ»؟', 'محمد', 'القصة'),
      q('المبتدأ في «العلمُ نورٌ»؟', 'العلم', 'نور'),
      q('الخبر في «الجوُّ جميلٌ»؟', 'جميل', 'الجو'),
      q('«الطالبان مجتهدان» — نوع الجملة؟', 'اسمية', 'فعلية'),
      q('علامة رفع المثنى؟', 'الألف', 'الواو'),
    ],
  },
  {
    ...
    totalMark: 20,
    questions: [
      q('مصدر الطاقة الذي يحرّك دورة الماء؟', 'الشمس', 'الرياح'),
      q('تحوّل الماء من سائل إلى بخار يسمّى؟', 'التبخّر', 'التكاثف'),
      q('تكوّن الغيوم ينتج عن عملية؟', 'التكاثف', 'التبخّر'),
      q('الماء الذي يتسرّب إلى باطن الأرض يسمّى؟', 'الماء الجوفي', 'الماء العذب'),
      q('أيٌّ ممّا يلي ليس من صور الهطول؟', 'الضباب', 'الثلج'),
      q('نسبة الماء العذب من ماء الأرض تقارب؟', '٣٪', '٣٠٪'),
    ],
  },
  {
    ...
    totalMark: 10,
    questions: [
      q('هل العدد ٣٤٥ يقبل القسمة على ٥؟', 'نعم', 'لا'),
      q('العدد الأولي من بين: ٩، ١٥، ١٧، ٢١؟', '١٧', '٩'),
      q('أصغر مضاعف مشترك للعددين ٤ و٦؟', '١٢', '٢٤'),
      q('كم مضاعفاً للعدد ٧ أقلّ من ٣٠؟', '٤', '٥'),
      q('كم قاسماً للعدد ١٢؟', '٦', '٤'),
      HARD_GCD,
    ],
  },
  {
    ...
    totalMark: 20,
    questions: [
      q('مجموع زوايا المثلث = ؟', '١٨٠°', '٣٦٠°'),
      q('محيط مربّع طول ضلعه ٥ سم؟', '٢٠ سم', '٢٥ سم'),
      q('٢٥٪ من ٨٠ = ؟', '٢٠', '٢٥'),
      q('الوسط الحسابي للأعداد ٤ و٦ و٨؟', '٦', '٩'),
      q('مساحة مستطيل بُعداه ٧ و٣ سم؟', '٢١ سم²', '٢٠ سم²'),
      q('أيّ الكسور أقرب إلى الواحد الصحيح؟', '٩/١٠', '١/٩'),
  ...
  pupils.forEach((pupil) => {
    ...
    ACTIVITIES.forEach((activity, ai) => {
      ...
      const items = missed.map((item, k) => {
        ...
        return { text: item.text, type: 'mc', ok: blank ? null : false, mine: blank ? '' : item.mine, right: item.right };
      });
